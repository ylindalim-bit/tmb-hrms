...
def _calc_working_days_from_pattern(db, year, month, work_pattern, base="MY"):
    ...
    Public Holidays list, scoped to this employee's own Base (MY/ZJ/CD -
    each site sets its own exact holiday dates, e.g. ZJ and CD's 2026
    Mid-Autumn/National Day arrangements differ by a day; a CD holiday
    should never reduce a ZJ or MY employee's working days, or vice
    ...
    holiday_rows = db.execute(
        ...
        (f"{year:04d}-{month:02d}-%", base or "MY"),
    ).fetchall()
    ph_days = 0
...
def attendance(year, month):
    ...
    for e in emps:
        ...
        suggestion = _calc_working_days_from_pattern(
            db, year, month, e["work_pattern"], e["base"]
        )
...
def holidays():
    ...
    if request.method == "POST":
        ...
        state = request.form.get("state") or "MY"
        if date and name:
    ...
    return render_template("holidays.html", holidays=rows, base_options=BASE_OPTIONS)
...
def portal_ot_claim():
    ...
    if request.method == "POST":
        ...
        ot_1_5, ot_2_0, ot_3_0 = _ot_claim_hours_from_form(db, request.form, claim_date, emp["base"])
        reason = request.form.get("reason", "").strip() or None
        if not claim_date:
...
@app.route("/ot-report/<int:year>/<int:month>")
def ot_report(year, month):
    """Per-employee OT totals for one payroll month, printable - same
    figures payroll itself reads (attendance_monthly: direct entry, or
    an approved OT Claim's hours once applied), so a still-Pending
    claim isn't counted until it's approved."""
    db = get_db()

    emps = employed_this_month(
        db, year, month,
        select_cols="emp_id, full_name, department, position, ot_approval_required",
    )

    rows = []
    for e in emps:
        totals = db.execute(
            """SELECT COALESCE(ot_hours_1_5,0) AS ot_1_5, COALESCE(ot_hours_2_0,0) AS ot_2_0,
                      COALESCE(ot_hours_3_0,0) AS ot_3_0
               FROM attendance_monthly WHERE emp_id=? AND year=? AND month=?""",
            (e["emp_id"], year, month),
        ).fetchone()
        ot_1_5 = totals["ot_1_5"] if totals else 0
        ot_2_0 = totals["ot_2_0"] if totals else 0
        ot_3_0 = totals["ot_3_0"] if totals else 0
        if ot_1_5 == 0 and ot_2_0 == 0 and ot_3_0 == 0:
            continue
        pending = db.execute(
            "SELECT COUNT(*) AS c FROM ot_claims WHERE emp_id=? AND status='Pending' AND claim_date LIKE ?",
            (e["emp_id"], f"{year:04d}-{month:02d}-%"),
        ).fetchone()["c"]
        rows.append({
            "emp_id": e["emp_id"], "full_name": e["full_name"], "department": e["department"],
            "position": e["position"], "ot_approval_required": e["ot_approval_required"],
            "ot_1_5": ot_1_5, "ot_2_0": ot_2_0, "ot_3_0": ot_3_0,
            "ot_total": ot_1_5 + ot_2_0 + ot_3_0,
            "pending": pending,
        })
...
def _is_rest_day(db, claim_date, base="MY"):
    ...
    check is scoped to this employee's own Base (MY/ZJ/CD), same as
    _calc_working_days_from_pattern."""
    try:
    ...
    if d.weekday() == 6:
        return True
    return db.execute(
        "SELECT 1 FROM public_holidays WHERE date=? AND state=?", (claim_date, base or "MY")
    ).fetchone() is not None


def _ot_claim_hours_from_form(db, f, claim_date, base="MY"):
...
def ot_claim_new():
    ...
    claim_date = request.form.get("claim_date", "")
    reason = request.form.get("reason", "").strip() or None
    emp = db.execute("SELECT base FROM employees WHERE emp_id=?", (emp_id,)).fetchone()
...
def hr_migrate_schema():
    ...
    daily_cols = [r[1] for r in db.execute("PRAGMA table_info(attendance_daily)").fetchall()]
    if "late_early_remark" not in daily_cols:
        # Why a Late in / Early out on that day isn't to be deducted (e.g.
    ...
    if "state" not in ph_cols:
        # Which employees.base (MY/ZJ/CD) this holiday applies to - each
        # site sets its own exact dates, so every existing row predates
        # this and was Malaysia's calendar, defaulting to 'MY' rather
        # than silently applying to China staff too.
        db.execute("ALTER TABLE public_holidays ADD COLUMN state TEXT NOT NULL DEFAULT 'MY'")
...
@app.route("/hr/import-halimah-august", methods=["POST"])
def hr_import_halimah_august():
    """One-time import: Halimah binti Hussin's (H001) real August 2026
    attendance from "202608-PASIR GUDANG-ATTN-HALIMAH.xlsx" - a complete
    day-by-day HR-prepared record. She joined 1 Aug 2026, so the whole
    month is either WORKED (08:30-17:30, Meal ticked - matches her
    already Y/RM8 profile), OFF (all 5 Saturdays), REST (Sundays), or
    PH (8/31, Merdeka Day) - no leave taken yet. Safe to re-run."""
    token = os.environ.get("RESTORE_TOKEN")
    if not token or request.form.get("token") != token:
        abort(404)
    db = get_db()
    ...
    days = {
        ...
        8: off, 9: rest, 10: worked, 11: worked, 12: worked, 13: worked, 14: worked,
        15: off, 16: rest, 17: worked, 18: worked, 19: worked, 20: worked, 21: worked,
        22: off, 23: rest, 24: worked, 25: worked, 26: worked, 27: worked, 28: worked,
        29: off, 30: rest, 31: ph,
    }
    for day, (day_type, time_in, time_out, meal_flag) in days.items():
        date_str = f"2026-08-{day:02d}"
        db.execute(
...
@app.route("/hr/fix-h001-l002-september-weekends", methods=["POST"])
def hr_fix_h001_l002_september_weekends():
    """One-time fix: H001 and L002 are both 'Weekly Fixed - 5-day
    (Mon-Fri)' - discovered (while investigating why their Daily
    Attendance page didn't show Sat/Sun as OFF/REST) that every Sat/Sun
    in September 2026 had already been saved as WORKED with no punch
    times, no meal, no reason - leftover from a whole-month Save done
    before the OFF/REST-default fix existed, not real attendance
    (confirmed nobody could have "worked" the still-future dates in
    that set). L002's Sept 6 and Sept 12 are excluded - those two carry
    real punch times, Meal, and a reason, i.e. genuine recorded weekend
    work, and are left untouched. Corrects the day_type on the affected
    rows only, then re-syncs attendance_monthly and re-finalizes
    September payroll for both so pay reflects the correction. Safe to
    re-run (each date is set to an explicit value, and finalize is
    idempotent)."""
    token = os.environ.get("RESTORE_TOKEN")
    if not token or request.form.get("token") != token:
        abort(404)
    ...
    fixes = {
        "H001": ["2026-09-05", "2026-09-06", "2026-09-12", "2026-09-13",
                  "2026-09-19", "2026-09-20", "2026-09-26", "2026-09-27"],
        "L002": ["2026-09-05", "2026-09-13", "2026-09-19", "2026-09-20", "2026-09-26", "2026-09-27"],
    }
    applied = []
    for emp_id, dates in fixes.items():
        for date_str in dates:
            weekday = datetime.date.fromisoformat(date_str).weekday()
            day_type = "REST" if weekday == 6 else "OFF"
            db.execute(
                "UPDATE attendance_daily SET day_type=? WHERE emp_id=? AND date=? AND day_type='WORKED'",
                (day_type, emp_id, date_str),
            )
            applied.append(f"{emp_id} {date_str} -> {day_type}")
    ...
    for emp_id in fixes:
        ...
        results.append(f"{emp_id}: re-finalized Sept 2026 (days_worked={result['working_days_in_month'] - result['unpaid_days'] - result['paid_leave_days']}, net_pay={result['net_pay']})")

    db.commit()
    return "OK - attendance fixed: " + "; ".join(applied) + " | payroll: " + "; ".join(results), 200


@app.route("/hr/fill-h001-september-full-attendance", methods=["POST"])
def hr_fill_h001_september_full_attendance():
    """One-time fix: H001's September 2026 WORKED rows (all weekdays,
    after the weekend OFF/REST correction above) had day_type set but no
    actual punch times - fills every WORKED weekday with her Normal
    Start/End Time (08:30-17:30) and ticks Meal Allowance (she's
    eligible). Also corrects Sept 16, which is Malaysia Day (Public
    Holiday) but had been left as WORKED, to PH. Leaves the already-
    correct OFF/REST weekend rows untouched. Re-syncs
    attendance_monthly and re-finalizes September payroll afterward.
    Safe to re-run."""
    token = os.environ.get("RESTORE_TOKEN")
    if not token or request.form.get("token") != token:
        abort(404)
    ...
    for day in range(1, 31):
        ...
        row = db.execute(
            "SELECT day_type FROM attendance_daily WHERE emp_id=? AND date=?", (emp_id, date_str)
        ).fetchone()
        if row is None or row["day_type"] in ("OFF", "REST"):
            continue
        if date_str in ph_dates:
            db.execute(
                "UPDATE attendance_daily SET day_type='PH', time_in=NULL, time_out=NULL, meal_allowance_flag='N' WHERE emp_id=? AND date=?",
                (emp_id, date_str),
            )
            applied.append(f"{date_str} -> PH")
        else:
            db.execute(
                "UPDATE attendance_daily SET day_type='WORKED', time_in='08:30', time_out='17:30', meal_allowance_flag='Y' WHERE emp_id=? AND date=?",
                (emp_id, date_str),
            )
...
@app.route("/hr/split-china-zj-cd-holidays", methods=["POST"])
def hr_split_china_zj_cd_holidays():
    """One-time fix: Public Holidays' state was first scoped using
    employees.holiday_state (Johor/China), but ZJ's own 2026 Mid-Autumn/
    National Day notice turned out to differ from CD's (ZJ: Mid-Autumn 1
    day, National Day Sept 30 + Oct 1-4; CD: Mid-Autumn 2 days, National
    Day Oct 1-5) - one shared "China" calendar isn't precise enough.
    Switched the scoping to employees.base (MY/ZJ/CD) instead, since
    that's already reliably set per employee and directly matches each
    site. Relabels existing rows (Johor -> MY, China -> CD, since the
    'China' rows seeded earlier were actually CD's specific dates), then
    seeds ZJ's own correct dates. Safe to re-run."""
    token = os.environ.get("RESTORE_TOKEN")
    if not token or request.form.get("token") != token:
        abort(404)
    db = get_db()
    db.execute("UPDATE public_holidays SET state='MY' WHERE state='Johor'")
    db.execute("UPDATE public_holidays SET state='CD' WHERE state='China'")

    zj_holidays = [
        ("2026-09-25", "Mid-Autumn Festival"),
        ("2026-09-30", "National Day"),
        ("2026-10-01", "National Day"),
        ("2026-10-02", "National Day"),
        ("2026-10-03", "National Day"),
        ("2026-10-04", "National Day"),
    ...
    applied = []
    for date_str, name in zj_holidays:
        if db.execute(
            "SELECT 1 FROM public_holidays WHERE date=? AND state='ZJ'", (date_str,)
        ).fetchone():
            continue
        day = datetime.date.fromisoformat(date_str).strftime("%A")
        db.execute(
            "INSERT INTO public_holidays (date, day, name, state) VALUES (?,?,?,'ZJ')",
            (date_str, day, name),
        )
        applied.append(f"{date_str} {name}")
    db.commit()
    return "OK - relabeled Johor->MY, China->CD; added ZJ: " + (
        "; ".join(applied) if applied else "(nothing new, already on file)"
    ), 200


@app.route("/hr/fix-manual-pattern-september-weekends", methods=["POST"])
def hr_fix_manual_pattern_september_weekends():
    """One-time fix: like H001/L002 before, these 4 employees are all on
    the 'Manual' work pattern (no auto OFF/REST default), and a handful
    of their September 2026 Sat/Sun rows were left over from a
    whole-month Save as WORKED with no punch time, no meal, nothing -
    not real attendance. Confirmed against each employee's actual
    August 2026 pattern (Sunday always REST; Saturday follows their own
    OFF/WORKED rotation) before picking OFF vs REST per date. Only
    touches rows that are still fully blank (day_type='WORKED' AND both
    time_in/time_out NULL), so it can't clobber a day that turns out to
    have real punch data (e.g. M001's Sept 12, which has a real
    time_in, was deliberately left out of this fix). Re-syncs
    attendance_monthly and re-finalizes September payroll for each
    affected employee afterward. Safe to re-run."""
    token = os.environ.get("RESTORE_TOKEN")
    if not token or request.form.get("token") != token:
        abort(404)
    db = get_db()
    ...
    fixes = {
        "A002": {"2026-09-05": "OFF", "2026-09-06": "REST", "2026-09-13": "REST"},
        "I001": {"2026-09-12": "OFF", "2026-09-13": "REST"},
        "L001": {"2026-09-05": "OFF", "2026-09-06": "REST", "2026-09-13": "REST"},
        "M004": {"2026-09-05": "OFF", "2026-09-06": "REST", "2026-09-13": "REST"},
    }
    applied = []
    for emp_id, dates in fixes.items():
        for date_str, day_type in dates.items():
            cur = db.execute(
                "UPDATE attendance_daily SET day_type=? WHERE emp_id=? AND date=? "
                "AND day_type='WORKED' AND time_in IS NULL AND time_out IS NULL",
                (day_type, emp_id, date_str),
            )
            if cur.rowcount:
                applied.append(f"{emp_id} {date_str} -> {day_type}")
...
@app.route("/hr/fill-k002-september-sundays", methods=["POST"])
def hr_fill_k002_september_sundays():
    """One-time fix: K002 (Manual work pattern) has had no attendance
    saved at all for most of September 2026 (only Sept 2-3 MC exist),
    so every Sunday shows amber/unrecorded rather than REST - confirmed
    with HR that Sunday is her rest day. Fills in the past Sundays only
    (Sept 6, 13, 20 - Sept 27 hasn't happened yet, left alone) as REST.
    Only inserts a row where none exists yet, so it can't clobber real
    data. Re-syncs attendance_monthly and re-finalizes September
    payroll afterward. Safe to re-run."""
    token = os.environ.get("RESTORE_TOKEN")
    if not token or request.form.get("token") != token:
        abort(404)
    db = get_db()
    ...
    for date_str in sundays:
        cur = db.execute(
            "INSERT INTO attendance_daily (emp_id, date, day_type) "
            "SELECT ?, ?, 'REST' WHERE NOT EXISTS "
            "(SELECT 1 FROM attendance_daily WHERE emp_id=? AND date=?)",
            (emp_id, date_str, emp_id, date_str),
        )
        if cur.rowcount:
            applied.append(f"{emp_id} {date_str} -> REST")
...
@app.route("/hr/fill-w001-w002-september-attendance", methods=["POST"])
def hr_fill_w001_w002_september_attendance():
    """One-time fix: W001 and W002 (both Manual work pattern, base MY)
    had zero September 2026 attendance saved at all - confirmed with HR
    they worked Mon-Sat their Normal Start/End Time (W001 07:30-17:30,
    W002 08:30-17:30), Sunday is REST, and Sept 16 (Malaysia Day) is PH.
    Only fills days up to today (the rest of the month hasn't happened
    yet). Neither is Meal/CEWI eligible so those stay unticked. Only
    inserts a row where none exists yet, so it can't clobber real data.
    Re-syncs attendance_monthly and re-finalizes September payroll for
    both afterward. Safe to re-run."""
    token = os.environ.get("RESTORE_TOKEN")
    if not token or request.form.get("token") != token:
        abort(404)
    db = get_db()
    # Hardcoded to "today" as of when this fix was written (2026-09-18),
    # not datetime.date.today() - the real server clock is nowhere near
    # this app's 2026 test data, so it can't be used to bound "so far".
    last_day = datetime.date(2026, 9, 18)
    days_to_fill = (last_day - datetime.date(2026, 9, 1)).days + 1

    times = {"W001": ("07:30", "17:30"), "W002": ("08:30", "17:30")}
    applied = []
    for emp_id, (start, end) in times.items():
        for day in range(1, days_to_fill + 1):
            date_obj = datetime.date(2026, 9, day)
            date_str = date_obj.isoformat()
            if date_obj.weekday() == 6:
                day_type, time_in, time_out = "REST", None, None
            elif date_str == "2026-09-16":
                day_type, time_in, time_out = "PH", None, None
            else:
                day_type, time_in, time_out = "WORKED", start, end
            cur = db.execute(
                "INSERT INTO attendance_daily (emp_id, date, day_type, time_in, time_out) "
                "SELECT ?, ?, ?, ?, ? WHERE NOT EXISTS "
                "(SELECT 1 FROM attendance_daily WHERE emp_id=? AND date=?)",
                (emp_id, date_str, day_type, time_in, time_out, emp_id, date_str),
            )
            if cur.rowcount:
                applied.append(f"{emp_id} {date_str} -> {day_type}")
    ...
    db.commit()
    return "OK - attendance filled: " + (f"{len(applied)} day(s) inserted" if applied else "(nothing to fill, already applied)") + " | payroll: " + "; ".join(results), 200


@app.route("/hr/fill-s001-september-sundays", methods=["POST"])
def hr_fill_s001_september_sundays():
    """One-time fix: S001 (Manual pattern, base CD) has real punch data
    for Sept 9-12 and 14-17, but nothing at all for Sept 1-8 or Sept 13
    (a Sunday) - confirmed with HR that Sunday is her rest day, so fills
    just that Sunday (Sept 6 too, also blank) as REST. Sept 1-5, 7, 8
    are weekdays with no confirmed data and are deliberately left alone
    - only the confirmed rest day is being filled here. Only inserts a
    row where none exists yet, so it can't clobber real data. Re-syncs
    attendance_monthly and re-finalizes September payroll afterward.
    Safe to re-run."""
    token = os.environ.get("RESTORE_TOKEN")
    if not token or request.form.get("token") != token:
